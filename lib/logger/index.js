...
const TAG = {
		REQUEST: 'talko-request',
		RESPONSE: 'talko-response',
		DEBUG: 'talko-debug',
		ERROR: 'talko-error',
		MESSAGE: 'talko-message'
	},
	...
	if(process.env.NODE_ENV === 'development' || process.env.NODE_ENV){
		...
		streams = [{
				...
				stream: bunyanDebugStream({
					basepath: '../../app.log' // this should be the root folder of your project.
				})
			}, {
				level: 'warn',
				type: 'raw',
...
function requestLogger(req, res, next){
	...
	res.req = req;
	log(TAG.REQUEST, ['======================= [time] Request ID : ', req.seq , ' ===========================']);
...
function log(tag, data){
	...
	logger.info(_getLoggableData(data))
}
